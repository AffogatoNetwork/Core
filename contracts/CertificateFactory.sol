...
contract CertificateFactory is Ownable, Pausable {
    ...
    event LogAssignCertificate(address _ownerAddress, address _certifierAddress, uint _coffeeBatchId, uint _certificateId);
    ...
    function getCertificateById(uint _certificateId) public view returns (uint, bytes32, string memory, string memory, string memory) {
        Certificate memory certificate = certificates[_certificateId];
        return (certificate.uid, certificate.name, certificate.imageHash, certificate.description, certificate.additionalInformation);
    }
