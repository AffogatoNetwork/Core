...
contract(Actor,function(accounts){
	...
	it('Adds a Processor',function(){
		return Actor.deployed().then(function(instance){
			...
			assert.equal(receipt.logs.length, 1, 'triggers one event');
      		assert.equal(receipt.logs[0].event, 'LogAddProcessor', 'should be the "LogAddProcessor" event');
            ...
            assert.equal(byteToString(processor[1]),"Cafetano", 'name is equal to inserted');
            assert.equal(byteToString(processor[2]), "Taster", 'type is equal to inserted');
            assert.equal(byteToString(processor[3]), "Francisco Morazan", 'department is equal to inserted');
            assert.equal(byteToString(processor[4]), "Honduras", 'country is equal to inserted');
            ...
            assert.equal(processor[7], '{"story":"Tostaduría de Café Especial, donde se sirve el mejor café de la ciudad.","experience":"Specialty Coffee Association of America Certificate","website":"https://www.facebook.com/cafetanohn/"}');
        });
    });
    ...
    it('Updates a Processor',function(){
		return Actor.deployed().then(function(instance){
			...
			assert.equal(receipt.logs.length, 1, 'triggers one event');
      		assert.equal(receipt.logs[0].event, 'LogUpdateProcessor', 'should be the "LogUpdateProcessor" event');
      		assert.equal(receipt.logs[0].args._id.toNumber(), 0, 'logs the inserted address id');
      		return tokenInstance.getCount();
		}).then(function(count){
            ...
            assert.equal(processor[0], accounts[1], 'account is equal to inserted');
            assert.equal(byteToString(processor[1]),"Cafetano", 'name is equal to inserted');
            assert.equal(byteToString(processor[2]), "Coffee Shop", 'type is equal to inserted');
            assert.equal(byteToString(processor[3]), "Francisco Morazan", 'department is equal to inserted');
            assert.equal(byteToString(processor[4]), "Honduras", 'country is equal to inserted');
            assert.equal(processor[5], 141058776000, 'lat is equal to inserted');
            assert.equal(processor[6], -871768649000, 'lon is equal to inserted');
